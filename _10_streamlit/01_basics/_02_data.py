...
# 특히 구조화된 데이터(표 혈식) 처리에특화
# DataFrame이라는 구조ㅡㄹ 중심으로 빠르고 직관적인 데이터 처리 및 분석을 지원한다.
...
st.subheader(body = "Dataframe + csv",divider=True)
sample_df = pd.read_csv('../data/annual-enterprise-survey-2023.csv')
st.dataframe(sample_df)

st.subheader('DataFrame 강조 기능',divider='blue')
data = {
    "Product" : ["A","B","C","D"],
    "Sales" : [500,300,400,600],
    "Growth(%)" : [10,-5,15,7]
}
df= pd.DataFrame(data) #DataFrame 객체생성
